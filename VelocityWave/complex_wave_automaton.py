...
class WaveAutomaton:
    def __init__(self, state_width=50):
        ...
        self.state = np.linspace(-0.1, 0.1, state_width, dtype=np.cdouble)
        self.state[0 * self.state_width // 5 : 1 * self.state_width // 5] = 0.01
        # self.state[2 * self.state_width // 5: 3 * self.state_width // 5] = 0.002
        self.velocity = np.linspace(-0.02j, 0.03j, state_width, dtype=np.cdouble)
    ...
    def step(self, timestep=0.1):
        ...
        self.velocity -= self.state / np.maximum(np.sqrt(state_magnitude), 0.001) * timestep / np.clip(state_magnitude, 0.01, 1)
        self.velocity = blur(self.velocity, 0.05)
        # self.velocity -= (neighbor_averages(self.velocity) - self.velocity) * timestep
        self.state += np.maximum(self.velocity * timestep, 0.01)
        self.state = blur(self.state, 0.05)
