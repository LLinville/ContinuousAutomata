...
def neighbor_averages(values, width=5):
    ...
    for i in range(1, width):
        total += np.roll(values, -i)/i + np.roll(values, i)/i
...
def neighbor_pull(values):
    ...
    for i in range(1, window_width // 2 + 1):
        for p in [-1, 1]:
            difference = np.roll(values, p * i) - values
            norm = np.linalg.norm(difference, axis=0)
            total_pull += difference * force_at_distance(norm) / norm / i * window_width
...
class Automaton:
    def __init__(self, state_width=50):
        ...
        seed = 103
        self.attractor = chua_attractor2
        start_range = (-2, 2)
        self.state = np.stack((
            np.array([scaled_raw_noise_2d(*start_range, seed*10000, t) for t in np.linspace(0, 20.5, state_width)]),
            np.array([scaled_raw_noise_2d(*start_range, seed*20000, t) for t in np.linspace(0, 20.5, state_width)]),
            np.array([scaled_raw_noise_2d(*start_range, seed*30000, t) for t in np.linspace(0, 20.5, state_width)])
        ))
        # self.state = np.array([[0.1], [0.3], [-0.6]]) * np.ones_like(self.state)
    ...
    def step(self, timestep):
        ...
        self.neighbor_pull_strength = 0.001
